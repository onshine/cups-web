...
//  2. cidfmap 文件存在（Docker runtime）→ 返回 1 个参数：-I<搜索路径>
//     gs 10.x 中 cidfmap 已安装到 Resource/Init/cidfmap，自动加载，无需额外参数
//
...
func TestCidfmapPreambleArgs(t *testing.T) {
	...
	// 分支 2：在临时目录里建一个假的 cidfmap.local，preamble 必须含 1 个参数
	tmpDir := t.TempDir()
