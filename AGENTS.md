...
> - `maven:3.9-eclipse-temurin-17`：Eclipse Temurin 对 "Linux ARM 32-bit Hard-Float" 仅 JDK 8/11 有二进制，JDK 17/21/25 [官方明确 Not Supported](https://adoptium.net/supported-platforms)；Maven 官方镜像同样没有 armhf manifest。**替代方案**：以 `debian:bookworm-slim` 为基础 apt 安装 `openjdk-17-jdk-headless` + `ca-certificates` + `curl`，Maven 则用 **Apache 官方 3.9.x 二进制 tarball**（纯 Java，同一个 tar.gz 跨架构通吃），`ofd-converter` 的 `maven.compiler.source=1.8` 与 JDK 17 完全兼容。**为什么不用 Debian 的 `apt install maven`**：QEMU 用户态模拟 armhf 下，Debian 的 `maven` 包装完无法启动，会抛 `java.lang.ClassNotFoundException: org.apache.maven.cli.MavenCli`（上游已知坑，见 [carlossg/docker-maven#213](https://github.com/carlossg/docker-maven/issues/213)）。根因是 Debian `maven` 包依赖 `dpkg triggers + update-alternatives` 把 `maven-core.jar` 等软链进 `plexus-classworlds` 启动器的 classpath，QEMU 模拟下这些 triggers 偶尔会静默跳过或执行不完整；Apache 官方 tarball 的 `lib/` 自包含所有 jar，不依赖任何 OS 打包细节，所以一劳永逸。代价是 java-builder 阶段体积略大（多一份 Maven 安装 + 本地 `~/.m2` 依赖缓存），但因 `builder` / `runtime` 阶段不会把它 `COPY` 进最终镜像（只 COPY 产物 `ofd-converter.jar`），所以**不影响最终镜像大小**。升级 Maven 时改 `Dockerfile` 里的 `MAVEN_VERSION` 即可。
